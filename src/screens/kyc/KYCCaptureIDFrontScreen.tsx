...
type Nav = NativeStackNavigationProp<KYCFlowStackParamList, 'KYCCaptureIDFront'>;
...
export function KYCCaptureIDFrontScreen() {
  const navigation = useNavigation<Nav>();
  const { colors: themeColors } = useTheme();
  const { draft, setDraft, setLastRecord } = useKYCFlow();

  const [loading, setLoading] = useState(false);
  const [previewUri, setPreviewUri] = useState<string | null>(draft.idFrontUri || null);

  const requiresBack = useMemo(() => draft.documentType !== 'passport', [draft.documentType]);
  ...
  const pick = async (source: 'camera' | 'gallery') => {
    ...
    const uploadFile = buildUploadFile(asset, 'id-front');
    setLoading(true);
    try {
      const res = await kycService.uploadDocument({
        kyc_id: draft.kycId,
        document_role: 'id_front_image',
        file: uploadFile,
      });
      setLastRecord(res.data?.record || null);
      setPreviewUri(asset.uri);
      setDraft((prev) => ({ ...prev, idFrontUri: asset.uri }));

      if (requiresBack) {
        navigation.navigate('KYCCaptureIDBack');
      } else {
        navigation.navigate('KYCLivenessSelfie');
      }
    } catch (error) {
      const userMessage = getUserMessage(error, 'kyc.upload.id_front');
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: themeColors.background }]}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={[styles.title, { color: themeColors.text }]}>Capture ID (Front)</Text>
        <Text style={[styles.subtitle, { color: themeColors.textSecondary }]}>
          Make sure the entire document is visible, in focus, and free of glare.
        </Text>
